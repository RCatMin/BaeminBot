...
> **한글 커맨드가 거부된다면?**
> 슬랙이 `/점심팟` 같은 한글 이름을 막는 경우가 있습니다. 그럴 땐 매니페스트에서
> `/점심팟` → `/lunch`, `/계좌등록` → `/account` 로 바꾸세요.
> 봇 코드가 **두 이름을 모두 받도록** 되어 있어서 그대로 동작합니다.
...
| "채널에 메시지를 올리지 못했어요" DM이 옴  | 비공개 채널이라면 `/invite @점심팟봇` 으로 봇을 초대해야 합니다.             |
